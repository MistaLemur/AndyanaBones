...
atom
	...
	proc
		...
		height(qx,qy,qw,qh)
			...
			var/sx
			if(pright > pleft)
				sx = min(pwidth , qx + qw - px)

			else
				sx = max(0,qx - px)


			. = py + pright * (sx/pwidth) + pleft * (1-sx/pwidth)
			*/
			////world<<"[sx]:[mx], [pleft], [pright] : [.]|"
	...
	proc
		...
		pixel_move(dpx, dpy)
			...
			if(stick_to_ramp && move_y <= 0)
				...
				var/groundTurfs[] = bottom(0.25 * icon_height)
				var/iterations = 0
				for(var/turf/t in groundTurfs)
					...
					if(abs(h - py) > (0.25 * icon_height)) continue

					iterations ++
					if(iterations == 1) move_y = h - py
					else
						if(abs(h - py) < abs(move_y)) move_y = h-py
		...
		set_pos(nx, ny, map_z = -1)
			...
			if(moved || on_ground || was_on_ground)
				...
				for(var/atom/a in obounds(src, 0, -1, 0, -pheight + 1))
					...
					if(a.pleft != a.pright)
						if(abs(py - a.height(px,py,pwidth,pheight)) > 0)
							continue
					else
						if(py != a.py + a.pheight)
							continue
